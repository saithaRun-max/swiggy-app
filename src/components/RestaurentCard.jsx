...
const RestaurentCard = (props) => {
  ...
  console.log(props);
  return (
    <div className=" min-h-[320px] max-w-60 m-5 mt-6 shadow-xl rounded-lg p-1 ">
      {
        <img
      ...
      <h5 className="text-slate-500 flex pl-2  flex-wrap">
        {cuisines.map((cusine, index) => (
          <h4 className="" key={index}>{cusine},</h4>
        ))}
      </h5>
      <p className="text-slate-700 block mt-2 pl-2 ">{locality + ", " + areaName}</p>
    </div>
  );
};
